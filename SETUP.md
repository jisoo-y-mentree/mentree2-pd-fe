...
### 훅 자기검사는 실행 위치에 따라 거짓 실패한다

`guard-git.test.sh` 는 **git 리포 안 ＋ feature 브랜치**를 전제한다.
...
**이 실패를 보고 훅을 고치지 않는다.** 통과 개수는 조건부 블록 때문에 환경마다 다르다. **판정 기준은 「실패 0」이다.**
