...
export function InterviewCard({
  ...
  return (
    <article
      ...
      {...rest}
    >
      {/* 미디어 3:2 — MentorCard와 같은 인셋/radius. 배지 2개(국가+직무)는 좌상단 오버레이. */}
      <div style={{ position: "relative", margin: "var(--card-media-margin) var(--card-media-margin) 0", aspectRatio: "3 / 2", background: "var(--sage-100)", borderRadius: "var(--card-media-radius)", overflow: "hidden" }}>
        {showImg ? (
        ...
        <div style={{ position: "absolute", left: 8, top: 8, right: 8, zIndex: 2, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", overflow: "hidden" }}>
          {(country || countryLabel) && <Badge size="sm" leading="flag" flag={country} style={{ boxShadow: "var(--shadow-sm)" }}>{countryLabel}</Badge>}
      ...
      <div style={{ padding: "var(--card-content-padding)", display: "flex", flexDirection: "column", gap: 8 }}>
        <a href={href} className="mt-card-link" style={{ display: "block", fontSize: "var(--text-h3)", fontWeight: 600, letterSpacing: "0.01em", lineHeight: 1.35, ...clamp(2) }}>{title}</a>
        {excerpt && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 6, flexWrap: "wrap" }}>
            <span style={{ fontSize: "var(--text-body)", fontWeight: 600, color: "var(--foreground)" }}>{personName}</span>
            {jobTitle && <span style={{ fontSize: "var(--text-caption)", fontWeight: 500, color: "var(--muted-foreground)" }}>{jobTitle}</span>}
          </div>
          {company && <span style={{ fontSize: "var(--text-caption)", fontWeight: 400, color: "var(--muted-foreground)" }}>{company}</span>}
        </div>
      </div>
    </article>
  );
