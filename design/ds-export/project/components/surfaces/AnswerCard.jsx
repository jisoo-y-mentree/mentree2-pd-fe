...
function MenuPopover({ open, onClose, anchorStyle, children }) {
  ...
  return (
    <div
      ...
      role="menu"
      style={{
        position: "absolute", zIndex: 30, minWidth: 168, display: "flex", flexDirection: "column",
        padding: 4, borderRadius: "var(--radius-md)", background: "var(--card)", border: "1px solid var(--border)",
        boxShadow: "var(--shadow-md)", ...anchorStyle,
      }}
    >
      {children}
    </div>
    ...
    >
      ...
      {isMine && (
        <div style={{ position: "absolute", top: 14, right: 14, zIndex: 2 }}>
          <IconButton icon="more-horizontal-circle-01" variant="ghost" size="sm" ariaLabel="더보기" onClick={() => setMoreOpen((v) => !v)} />
          <MenuPopover open={moreOpen} onClose={() => setMoreOpen(false)} anchorStyle={{ top: "calc(100% + 4px)", right: 0 }}>
            <button type="button" style={menuItemStyle} onClick={() => { setMoreOpen(false); onEdit && onEdit(); }}>수정</button>
            <button type="button" style={{ ...menuItemStyle, color: "var(--destructive)" }} onClick={() => { setMoreOpen(false); onDelete && onDelete(); }}>삭제</button>
          </MenuPopover>
        </div>
      )}
      ...
      <div style={{ position: "relative", zIndex: 2, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ position: "relative" }}>
            <IconButton icon="share-08" variant="ghost" size="sm" ariaLabel="공유" onClick={() => setShareOpen((v) => !v)} />
            <MenuPopover open={shareOpen} onClose={() => setShareOpen(false)} anchorStyle={{ top: "calc(100% + 4px)", left: 0 }}>
              <button type="button" style={menuItemStyle} onClick={copyLink}>
