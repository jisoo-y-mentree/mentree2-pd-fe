...
3. **본문** — 전문 그대로. `--text-article` 17/lh 28 — **읽을거리 본문**이다(스크롤해서 읽는다. 이 디자인 시스템의 `article` 첫 사용처). **line-clamp 없음**(QnaCard와 다름). 禁則 유지.
   한 줄 소개(헤드라인)는 반대로 `--text-caption` 14 — 카드 안 보조다.
4. **날짜** — 연월일만(시간 없음).
