...
4. **날짜** — 연월일만(시간 없음).
5. **액션 줄 — 한 줄, 좌우 양끝 정렬.** 좌: 도움돼요(`CountToggle tone="rose"`) · 공유(IconButton, 팝오버로 "이 답변
   링크 복사"). 우: 채택 배지 + 감사인사 보내기(Button primary sm) — 둘은 갈라놓지 않고 항상 함께 움직인다(좁은 폭에서
...
8. **더보기(⋯)** — 내가 쓴 답변에만, 카드 우상단(헤더 링크 위 z-index로 독립 클릭). 팝오버로 "수정"·"삭제". 남의 답변에는 안 뜬다.
