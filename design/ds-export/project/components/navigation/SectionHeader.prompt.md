...
<SectionHeader
  icon="user-group"
  title="지금 주목받는 " titleAccent="Q&A 멘토링"   {/* 강조가 뒤 — 기본 */}
  suffix="3,200명의 멘토"
...
- **좌(제목 영역)**: 장식 아이콘 칩(옵션, HugeIcons·radius 12·sage-50 배경·sage-200 아웃라인, IconButton 아님) + 제목(`--text-h0` semibold, `titleAccent`로 green 강조 조각 — 기본은 제목 **뒤**, `accentFirst`면 **앞**) + 부가 스트링(옵션, muted) + Badge(옵션). 부가 스트링·배지는 제목과 높이 가운데 정렬. 아이콘/부가스트링/배지는 각각 독립 슬롯.
...
- **`accentFirst`**: 강조가 앞에 오는 제목을 위한 것이다 — 「**멘토의 이야기**를 읽어보세요」는 `titleAccent="멘토의 이야기"` ＋ `title=" 를 읽어보세요"` ＋ `accentFirst`. 기본 `false`라 기존 호출은 하나도 안 바뀐다.
