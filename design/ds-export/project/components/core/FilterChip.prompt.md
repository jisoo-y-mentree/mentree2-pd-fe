...
선택 가능한 **필터 칩**. Toggle 계열 — CountToggle과 같은 Toggle 조상을 공유하는 형제다
(아이콘형=CountToggle / 텍스트칩형=FilterChip). 정적 표시 전용인 **Badge와 구분**된다:
FilterChip은 클릭·선택이 가능한 인터랙션 요소.

## 언제 쓰나
- 국가·직무 등 목록을 좁히는 **필터 칩 줄**. 다중 선택 토글.
- 단일 정적 라벨/분류에는 Badge를, 저장(스크랩) 아이콘 토글에는 CountToggle을 쓴다.
