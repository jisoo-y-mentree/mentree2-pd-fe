...
아이콘 + 카운트가 붙은 토글. **"도움돼요"와 "스크랩"이 이 하나로 통일**된다 — 거동은 완전히 같고 `icon`만 다르다.

## 카운트는 선택이다

`count`를 넘기지 않으면 **아이콘만** 뜬다. 카운트가 붙는 자리와 안 붙는 자리를 이 하나로 덮는다 —
**스크랩의 선택 표현을 전 화면에서 통일**하기 위해 옛 아이콘 전용 토글을 이 컴포넌트로 흡수했다(2026-09-11, 그 부품은 폐지).
스크랩은 멘토·질문·아티클을 넘나들며 마이페이지 한 곳에 모이므로, 누른 모습이 둘이면 같은 기능으로 읽히지 않는다.
...
- **사진 위에 얹을 때** — 배경이 없으므로 반투명 받침이나 그림자로 읽히게 한다(MentorCard 미디어 오버레이).
  배경을 채우는 반전으로 되돌리지 않는다.

## 상태

`default · hover(--sage-700) · active(0.5px 눌림) · selected(아이콘 fill + --primary, 배경 없음) · focus-visible(--ring) · disabled`
