...
| 판정 조건 | 아크션 |
|---|---|
| 「이 멘토의 다른 답변 보기」를 누른다 | **멘토 상세의 「Q&A멘토링 활동」 탭으로 보낸다. 새 창이다.** 전용 화면을 만들지 않는다 |
...
| 2 | **이벤트 상세** | 진입로가 3개(TOP 특설 버튼 · 알림 쿠폰 배너 · 비로그인 천이도)인데 **와이어가 없다.** 아티클 상세의 카테고리로 흡수할지도 미정 | 디자이너 | 마지막 차례 |
...
| 6 | **헤더 좌측이 2종이다** | shadcn 계통은 3메뉴, 1.0 계통 멘토 상세는 **검색창 ＋ 「멘토 전체보기」 ＋ 우하단 플로팅 벨**이다. 요소의 수도 종류도 다르므로 **UI 가 아니라 구성의 차이**다 | 디자이너 | `mentor-detail` |
