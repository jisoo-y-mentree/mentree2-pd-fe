...
| `SectionHeader` ＋ `QnaCard` ×4 | 「이런 질문도 많이 봤어요」 | 추천 질문 4장 | **`Carousel` 부품에 담는다**(2026-09-16 · 인사이트 상세와 같다). **답변 완료된 질문 중에서** 고른다. 겹치는 태그 기준(키워드 1순위 · 국가 2순위). **멘토에게는 「아직 답변이 등록되지 않은 질문들이에요」 — 답변 0건 질문**(와이어 `1059:38262`) |
