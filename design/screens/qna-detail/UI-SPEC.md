...
- 상태: 확정 (§1~11 ＋ `template/` 취임 2026-09-14 · **2026-09-15 개정** — Figma 라이브 대조 ＋ 결정 6건)
- 화면 구분: 열람
...
| 3 | 답변 수 | 「N건의 답변」. **정렬 드롭다운은 두지 않는다**(2026-09-15 결정 — 프레임에 없다) |
| 4 | **답변 목록** | 이 화면의 본체. **전부 등재한다** |
| 5 | 답변 유도 띠 ＋ **답변 작성 카드** | **멘토에게만.** 상단은 버튼이 아니라 안내 띠다(2026-09-15 결정) |
...
**두 형태는 같은 사양을 쓴다.** 폭이 다를 뿐 담기는 것과 규칙이 같다. **사이드에서 빠지는 것은 6·7·8번 블록이다** — 추천·배너·인기 태그는 목록으로 돌아가면 되므로 좁은 패널에 넣지 않는다.

**간단보기도 전체 페이지와 같은 갈래로 나뉜다**(2026-09-15). 멘티 · 질문자 · 멘토 × 답변 있음 · 0건으로 **6장**이다. 역할마다 담기는 것이 다르다(§8) — 질문자는 감사인사와 수정·삭제를, 멘토는 안내 띠와 답변 작성 카드를 갖는다. **「전체 페이지에서 보기」는 제 짝의 전체 페이지로 간다.** 한 장으로 두면 멘토가 눌러도 멘티 화면이 열린다.
...
| `IconButton` ＋ `Breadcrumb` | 되돌아가기 | ← ＋ 「Q&A 멘토링 ＞ 상세」 | **앞에 ← 를 둔다**(와이어). 문구는 「Q&A 멘토링」 유지(2026-09-15 결정). **우측에 「간단보기 ↗」** — 사이드 패널로 간다. 사이드 패널에서는 **「전체 페이지에서 보기 ↗」**(2026-09-15 정정 — 09-14 에 「전체보기」로 잘못 적었다) |
| — | **질문 블록** | 아래 「질문 블록의 구성」 | **카드 셸에 담지 않는다.** 맨바닥에 놓고 액션 줄 아래에 구분선 하나(2026-09-15 결정 — 와이어). 초록 테두리의 답변 카드와 시각으로 갈린다 |
| — | 답변 수 | 「N건의 답변」 | **N 은 실제 수다.** 정렬 드롭다운은 두지 않는다(2026-09-15). 순서는 서버 가중치(§8) |
| `AnswerCard` ×N | **답변 목록** | 부품이 구성을 갖는다 | **전부 등재한다**(§3). 쪽으로 나누지 않는다 |
| `CalloutBar` ＋ 답변 작성 카드 ＋ `Button` | 답변 유도 | 띠 「이 질문에 대해 멘토님의 지혜를 나눠주세요」 ＋ 에디터 카드 ＋ 하단 플로팅 「이 질문에 답변하기」 | **멘토에게만**(§8). 상단은 띠, 버튼은 플로팅 1곳(2026-09-15 결정). 작성 카드에는 「답변 등록하기」 하나 — 임시 저장은 두지 않는다 |
| `SectionHeader` ＋ `QnaCard` ×4 | 「이런 질문도 많이 봤어요」 | 추천 질문 4장 | **모바일에서는 세로로 쌓지 않고 `Carousel` 로 옆으로 넘긴다**(`P-05` · 2026-09-16). **답변 완료된 질문 중에서** 고른다. 겹치는 태그 기준(키워드 1순위 · 국가 2순위). **멘토에게는 「아직 답변이 등록되지 않은 질문들이에요」 — 답변 0건 질문**(와이어 `1059:38262`) |
| `Banner` | 질문 유도 | 연초록 면 ＋ 초록 테두리 · 글 2줄 ＋ 버튼 가운데 | 추천 섹션 **위**다. **멘토에게는 문구가 다르다** — 「멘토님, 다른 멘토에게 질문할 궁금증이 생기셨나요? 부담없이 질문 남겨보세요.」 |
...
| 3 | 작성자 | `Avatar`(이니셜) ＋ 이름. **`김＊＊` 로 마스킹한다.** 성만 남긴다 |
| 4 | 본문 | **전문.** 줄임 없음 |
| 5 | `Chip` ×N | 해시태그. **여기서는 5개로 접지 않는다** — 전부 세운다 |
| 6 | 액션 3개 | 도움돼요 · 스크랩 · 공유. **가운데 정렬한다** — 카드 하단의 좌측 정렬과 다르다. **숫자는 버튼에 붙인다**(`CountToggle`). 메타 줄에는 조회수·작성일만(2026-09-15 결정). 공유는 `Popover` 「링크 복사」 |
...
| 지금 보는 사람이 질문자다 | `isQuestionerView` 를 넘긴다 → 감사인사 버튼이 선다. 누르면 **`Dialog`**(글 한 칸 ＋ 취소·보내기) → 보내면 `Toast`(2026-09-15 결정 — 와이어에 화면이 없어 사람이 정했다) |
...
| **멘토** | 같은 문구 ＋ **안내 띠와 답변 작성 카드**(§8). 이 사람은 지금 답할 수 있다 |
...
| **지금 보는 사람이 질문자다** | 각 답변에 **채택(도움돼요)** 과 **감사인사**를 낸다. **답변이 0건일 때만** 질문 블록에 「수정하기」「삭제하기」 텍스트 링크를 낸다(와이어 `1059:40054` ＋ 주석) |
| 질문자가 답변에 도움돼요를 눌렀다 | **텍스트 배지**로 표시한다. **복수로 누를 수 있다** |
| **지금 보는 사람이 멘토이고 이 질문에 아직 안 답했다** | **안내 띠**(`CalloutBar`) ＋ **답변 작성 카드** ＋ **하단 플로팅** 「이 질문에 답변하기」를 세운다. 상단 버튼은 두지 않는다(2026-09-15 결정) |
| **멘토인데 이미 답했다** | 띠·작성 카드·플로팅을 **세우지 않는다.** 1인 1답이다. 내 답변 카드에 `isMine` |
| 답변을 수정했다 | **별도 표시를 하지 않는다.** 알림도 보내지 않는다 |
| 답변이 0건이다 | §7 의 빈 상태. **역할로 문구가 갈린다** |
| 답변의 순서 | **화면에서 바꾸지 않는다**(정렬 드롭다운 없음 · 2026-09-15). 서버 가중치는 **질문자 도움돼요 → 도움돼요 수 → 스크랩 수 → 오래된 → 최신** |
...
| **사이드 패널로 열렸다** | 추천 섹션·배너·인기 태그를 **빼고** 나머지는 같다(§4). **역할과 답변 수로 6장으로 갈린다** |
...
| 2 | ~~답변이 달린 뒤 질문을 수정·삭제할 수 있나~~ | **풀렸다** — 와이어 주석 「질문 등록 후 삭제: 답변 달리기 전에만 가능. 답변 달린 뒤엔 문의하기로 소화」(2026-09-15 확인) | — |
