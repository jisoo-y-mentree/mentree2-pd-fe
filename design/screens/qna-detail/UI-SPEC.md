...
- 화면 구분: 열람
- 템플릿: 미작성. **`Breadcrumb`(`DS-16`)를 기다린다** — 사양은 막지 않고 템플릿에서 막힌다
...
| 하지 않는 것 | 이유 |
|---|---|
| **답변마다 스크랩을 두지 않는다** | 스크랩의 목록 단위가 **질문**이다. 답변에 두면 한 질문에 여러 개가 동시에 켜진다. 와이어도 「답변 스크랩은 질문 스크랩과 같은 것으로 간주」라고 쓴다 |
...
| `Breadcrumb` | 되돌아가기 | 「Q&A 목록 ＜ 상세」 | **미구현**(`DS-16`). 이 화면이 기다리는 유일한 부품이다 |
...
| 6 | **사이드 패널에 URL 이 붙나** | 붙지 않으면 공유·뒤로가기가 전체 페이지로만 간다. transition-map 미결 5번(모달의 URL)과 같은 층이다 | 구현 인계 전 |
