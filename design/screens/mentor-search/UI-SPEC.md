...
| **`P-03`** 0건의 면에서 다음 행동을 낸다 | 「필터링 초기화」 ＋ **「이런 멘토는 어떠세요?」 추천 4장**을 낸다 |
...
| `FilterSelect` ×3 | 조건 | **국가 · 직종 · 키워드** | 누르면 **`Dialog` 가 열리고** 안은 **그룹으로 나눈** `FilterChip` 이다. 걸린 값을 트리거에 「일본 외 2」로 요약한다. 거동은 §8 |
| `SearchInput` | 검색창 | 직무 · 회사 · 키워드 · 이름 · 국가 **5종** | 자동완성 드롭다운 ＋ 최근 검색어. 거동은 §8. **최근 검색어를 키보드로 못 고른다**(`DS-29`) |
...
| 빈 상태 | **`EmptyState`**(`DS-11`) ＋ 「필터링 초기화」 ＋ **추천 4장** | `P-03`. 아래 |
| 에러 | 목록 자리에 사유와 재시도 버튼 | 아래 「에러 시의 거동」 |

**빈 상태가 이 화면의 볼거리다.** 「찾으시는 결과값의 멘토가 아직 없습니다」에서 끝내지 않고 **「이런 멘토는 어떠세요?」로 4장을 낸다.** 0건은 **조건이 좁은 것**이지 멘토가 없는 것이 아니다.

**와이어는 5장인데 4장으로 줄인다**(2026-09-13). `MentorCard` 가 **고정폭 296px 이고 리스폰시브가 아니라** 5장이면 1480px 이 필요해 1280 에 안 들어간다. 와이어는 카드 폭을 좁혀 다섯을 세웠는데 **그것은 부품을 깨는 것이다.** **본 목록과 같은 4열 그리드를 그대로 쓴다** — 카드 수 하나 때문에 `Carousel` 을 끌어오거나 `MentorCard` 를 고치지 않는다.

**추천 카드는 목록과 같은 `MentorCard` 다.** 와이어에는 「멘토 프로필 보기」 버튼이 붙어 있으나 **넣지 않는다** — 카드 전체가 이미 링크이고(`DS-22`), 버튼을 두면 4개의 버튼이 뜬다. 같은 판단을 `AnswerCard` 에서 이미 했다(`DS-23`).
...
| 결과가 0건이다 | **`EmptyState` ＋ 추천 4장**(`P-03`). 조건 줄과 검색어는 그대로 둔다 |
...
| 2 | **추천 4장의 선정 규칙** | 와이어가 「이런 멘토는 어떠세요?」까지만 적는다. 걸었던 조건을 한 축씩 푸는지, 인기순인지 아무도 정한 적이 없다 | 백엔드 합의 |
...
- [x] 그 부품이 `ds-export/` 에 들어왔다 (2026-09-13 · 43본)
