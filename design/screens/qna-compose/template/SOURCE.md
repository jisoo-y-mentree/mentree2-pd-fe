...
**6장.** normal · loading · error ＋ 모달 3종(phone · submit · leave). 임시 저장 모달은 2026-09-15 결정으로 뺐다
...
**규칙의 글자와는 어긋난다.** 판정을 사람에게 남긴다(PR 본문). 페이지 스크립트가 드는 인터랙션 상태도 같은 판정에 든다 — ADR-0005 의 문구를 「화면 상태를 토글하는 JS 를 남기지 않는다. 부품 인터랙션의 상태는 페이지 스크립트가 든다」로 개정할지 리뷰 때 정한다.
