...
| `Field` ＋ `Select` ＋ `RadioGroup` | 국가 | 드롭다운 ＋ 「모든 국가에 해당함」 | **라디오를 켜면 드롭다운을 비활성으로 바꾼다**(§8) |
...
| `Dialog` ×4 | 인증 안내 ／ 등록 확인 ／ 임시 저장 확인 ／ 이탈 방지 | — | 아래 「모달 4종」 |
| `Footer` | 전역 셸 | — | |
...
**와이어를 화면으로 대조했다**(`1059:35577`, 2026-09-14). **확인 모달이 셋이고 전부 「묻고 답하는」 형태다.**

| 언제 | 무엇 | 닫으면 |
|---|---|---|
| **화면에 들어온 직후** | **「전화번호 인증이 필요합니다. 지금 인증을 진행할까요?」** | **이전 화면으로 돌아간다.** 빈 폼을 보여주지 않는다 |
| 등록을 눌렀다 | 「질문을 등록하시겠습니까?」 | 머무른다 |
| 임시 저장을 눌렀다 | 「현 상태에서 임시 저장 하시겠습니까?」 | 머무른다 |
...
| 등록·임시 저장에 확인을 붙이는 이유 | **되돌릴 수 없거나 덮어쓴다.** 등록은 공개되고 임시 저장은 앞의 것을 덮는다 |
...
| 임시 저장을 눌렀다 | **확인 모달을 먼저 띄운다.** 앞의 저장을 덮기 때문이다 |
