...
| **임시 저장을 자동으로 하지 않는다** | 쓰다 만 질문이 본인도 모르게 쌓인다. **누를 때만 저장한다** |
...
| **미승격** — 쓰다 만 것을 잃지 않게 한다 | 이탈 시 파기 확인 ＋ 임시 저장. **답변 작성에는 이 규칙이 없다**(와이어가 안 적었다) — 2화면째를 기다린다 |
...
| `Field` ＋ `Select` ＋ `RadioGroup` | 국가 | 드롭다운 ＋ 「모든 나라/지역에 해당함」 | **라디오를 켜면 드롭다운을 비활성으로 바꾼다**(§8) |
| `Field` ＋ `Select` | 키워드 | 대분류 1개 | **목록은 미확정이다**([용어집](../../../docs/glossary/ubiquitous-language.md)). 값에 의존하는 판단을 쓰지 않는다 |
| `Field` ＋ `Input` | 제목 | **0/50자** | 카운터를 오른쪽에 붙인다 |
| `Field` ＋ **에디터 자리** | 본문 | **0/1000자** | 아래 「본문 에디터」 |
| `Field` ＋ `TagInput` | 태그 | **최대 10개** | 아래 「태그 입력」 |
| `Button` ×2 | 임시 저장 ／ 질문 등록하기 | — | 등록은 primary, 임시 저장은 secondary. **등록은 조건을 못 채우면 비활성**(§8) |
| `Dialog` | 전화번호 인증 ／ 이탈 방지 | — | 아래 「모달 2종」 |
| `Footer` | 전역 셸 | — | |
...
**`template/` 에서는 에디터 본체를 그리지 않는다.** 자리·툴바의 유무·글자 수 카운터만 보이면 된다.
...
| 언제 | 무엇 | 닫으면 |
|---|---|---|
| **화면에 들어온 직후** | 전화번호 인증 | **이전 화면으로 돌아간다.** 빈 폼을 보여주지 않는다 |
| **쓰던 중 이탈** | 「입력 내용 파기하시겠습니까?」 | 머무른다 |
...
| 로딩 | **등록 중.** 버튼을 비활성으로 바꾸고 진행을 보여준다 | 임시 저장 복원 중도 같다 |
...
| 판정 조건 | 아크션 |
|---|---|
| **화면에 들어왔다** | **전화번호 인증 모달을 즉시 띄운다.** 폼을 먼저 보여주지 않는다 |
| 인증 모달을 닫았다 | **이전 화면으로 돌려보낸다** |
| **「모든 나라/지역에 해당함」을 켰다** | **국가 드롭다운을 비활성으로 바꾼다.** 고른 값이 있으면 지운다 |
...
| 태그가 10개다 | 더 받지 않는다 |
| **쓰던 중 화면을 떠나려 한다** | 「입력 내용 파기하시겠습니까?」를 묻는다 |
| **아무것도 안 쓰고 떠난다** | **묻지 않는다.** 지킬 것이 없다 |
| 등록을 눌렀다 | 등록 버튼을 비활성으로 바꾸고 **중복 제출을 막는다** |
...
- `qna-compose` → 전화번호 인증: **들어오자마자.** 사람의 조작이 아니다
...
| 2 | **임시 저장이 몇 벌인가** | 한 벌만 덮어쓰는지 여러 벌인지 와이어가 안 적는다. 여러 벌이면 목록과 삭제가 필요하다 | 백엔드 합의 |
