...
- 화면 구분: 열람
- 템플릿: 아직 없다. `DS-55` · `DS-56` · `DS-34` · `DS-35` 가 디자인 시스템에 들어온 뒤 Claude Design 에서 만든다. `DS-54` · `DS-57` 은 아트디렉션 전까지 임시 조립이다(§6)
...
| — **`DS-54`** | 페이지 헤더 | `Avatar` · 이름 · 직무 @회사 · 연차 · 국기 ＋ 「<도시>에서 활동」 · 세계지도 ＋ 핀 | 풀폭. 핀 고정 · 지도 이동(§4). **이름과 직무를 말줄임하지 않는다** — 줄을 늘린다 |
...
| — **`DS-57`** | Q&A · 한 건 | 질문 한 줄 · 답변 발췌 · 반응 줄(도움돼요 · 조회 · 스크랩) | 카드 전체가 `qna-detail` 로 가는 링크다. **모양은 아트디렉션** |
...
**2026-09-24 에 `DS-54`〜`57` 로 기표했다.** `DS-38`〜`53` 은 지수 쪽이 Claude Design 에서 이미 쓴 번호라 건너뛰었다([DS-update-list.md](../../DS-update-list.md) `DS-57` 아래).

**`DS-54` · `DS-57` 은 아트디렉션 전까지 화면에서 임시 조립한다**(`data-temp`). 나머지 넷(`DS-55` · `DS-56` · `DS-34` · `DS-35`)은 디자인 시스템에서 먼저 만든다.

| 후보 | 무엇이 없나 | 판정 |
|---|---|---|
| **`DS-54`** 페이지 헤더(지도) | 풀폭 밴드 ＋ 세계지도 ＋ 고정 핀 ＋ 신원 블록. 도시에 맞춰 지도를 민다 | **기표한다.** 화풍은 아트디렉션에서 정한다 |
| **`DS-55`** `BottomTabBar` 의 스크롤 숨김과 위에 얹는 칸 | 스크롤로 숨는 거동이 없다. 신청 바를 얹을 자리가 없다 | **기표한다.** 신청 바를 칸에 얹어 한 부품이 두 줄을 함께 움직인다. 숨김은 켜는 화면에서만 동작한다 — 다른 화면은 그대로다 |
| **`DS-56`** 아티클 본문 | `article-detail` 은 본문을 화면에서 조립했다 | **기표한다.** 두 화면이 같은 글을 같게 그려야 한다. 2화면째다(원칙 7) |
...
- [ ] `DS-55` · `DS-56` · `DS-34` · `DS-35` 를 디자인 시스템에서 만든다
- [ ] `template/` 을 Claude Design 에서 만들어 취임한다 — `DS-54` · `DS-57` 은 `data-temp` 임시 조립
- [ ] 아트디렉션 뒤 `DS-54` · `DS-57` 을 부품으로 만들어 `data-temp` 자리를 갈아끼운다
