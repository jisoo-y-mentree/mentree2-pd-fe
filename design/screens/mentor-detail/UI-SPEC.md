...
- 상태: 확정 (§1~5 UX승인 2026-09-24 → §6~11 확정 2026-09-24 · 1 · 2차 export 검수 뒤 개정 2026-09-24)
- 화면 구분: 열람
- 템플릿: 아직 없다. Claude Design 에서 만드는 중이다 — 1 · 2차 export(2026-09-24)를 검수하고 구성을 고쳤다(§4 왼쪽 열 머리 · 탭 바 · 표지 · 소개글). `DS-54` · `DS-57` · `DS-67` 은 아트디렉션 전까지 임시 조립이다(§6)
...
| **신청 묶음을 전역 `Header` 에 넣지 않는다** | 그 자리는 로그인 · 알림 · 내 아바타 — 「내 계정」의 자리다. 「이 멘토에게 신청」은 왼쪽 열 머리에 둔다 |
...
┌ 왼쪽 열 (통째로 따라옴) ─────┬ 오른쪽 열 ─────────────────┐
│ ┌ 멘토 정보 ┐ ← 스크롤하면     │ 탭 바 (따라옴)              │
│ │ ● 박서연    │   위에서 내려와  │ 프로필 인터뷰 Q&A 7 리뷰 8  │
│ └ 프로덕트…  ┘   자리를 잡는다   │                            │
│ [1:1 멘토링 신청하기][관심][공유] │ 탭 본문                     │
│ 한 줄 요약                      │                            │
│ 인사이트 키워드                  │                            │
│ 커리어 · 학력 히스토리            │ 탭 끝 CTA 박스              │
└───────────────────────────────┴────────────────────────────┘
Footer
```
...
| 3 | **왼쪽 열 머리** | **신청 묶음**(1:1 멘토링 신청하기 ＋ 관심있는 멘토 ＋ 공유). 스크롤하면 그 위에 **멘토 정보**(아바타 · 이름 · 직무)가 내려온다 |
| 4 | 왼쪽 열 | 머리 · 한 줄 요약 · 인사이트 키워드 · 커리어 히스토리 · 학력 히스토리. **어느 탭에서든 보이고, 통째로 스크롤을 따라온다** |
| 5 | 탭 바 | 오른쪽 열 맨 위. **스크롤을 따라오고 왼쪽 열과 윗선이 같다** |
| 6 | 탭 본문 | 아래 「탭」 |
| 7 | 탭 끝 CTA 박스 | 다 본 사람에게 신청을 한 번 더 낸다 |
...
**신청 묶음은 왼쪽 열 머리에 두고, 탭 바는 오른쪽 열 맨 위에 둔다**(2차 export 검수 뒤 디자이너 판단 2026-09-24).

- 2차 export 는 이름 · 탭 · 신청 묶음을 멘토 바 한 줄에 몰았다. **좁은 폭에서 탭이 밀려 가려졌고**, 페이지 헤더가 보이는 동안 **이름 자리가 비어 있었다.** 그래서 한 줄을 왼쪽과 오른쪽으로 나눈다
- **왼쪽 열 머리는 처음에 신청 묶음만 선다.** 페이지 헤더가 화면에서 사라지면 **멘토 정보가 위에서 내려와 신청 묶음 위에 자리를 잡는다.** 신청 묶음은 그 아래로 밀리고 **계속 보인다** — 멘토 정보가 신청 묶음을 대신하지 않는다. 신청은 늘 손이 닿아야 한다
- 멘토 정보는 스크롤한 뒤에만 선다. 페이지 헤더가 보이는 동안에는 같은 이름이 두 번 서기 때문이다
- 신청 묶음은 모바일 하단 신청 바와 **같은 셋**이다
- 탭이 오른쪽 열 안에 있으니 **좁은 폭에서도 가려지지 않는다.** 769~1024px 에서 오른쪽 열은 440px 이상이고 탭 4개는 약 330px 이다

**왼쪽 열 전체가 `Header` 아래에 붙어 따라온다.** 긴 인터뷰와 답변을 읽는 동안 그 옆에 이 사람이 누구인지(이름 · 신청 · 경력)가 있다. 열이 화면보다 길면 끝까지 스크롤한 뒤 아래에 붙는다.

**한 줄 요약은 멘토카드의 소개(`intro`)와 같은 값이다.** 왼쪽 열 머리 바로 아래에 멘토카드와 같은 green-50 상자로 둔다. 검색 결과에서 본 한마디가 상세에서 이어진다.
...
- **「이어서 보기」를 누르면 인터뷰 탭으로 바꾸고, 탭 바가 화면 위에 붙는 자리까지 부드럽게 스크롤해 전문이 이어진다.** 페이지를 떠나지 않는다
...
페이지 헤더 — 지도 띠는 신원 블록까지만 · 핀은 오른쪽 세로 가운데
한 줄 요약 — 띠 아래 흰 바탕
━ 멘토 바 (따라옴) — 탭 줄 ＋ 그 위에 떠 있는 이름 줄 ━
탭 본문
─ 하단 고정: 신청 바 ／ BottomTabBar(아래로 스크롤하면 숨는다) ─
```

**왼쪽 열이 없으므로 그 내용이 프로필 탭 안으로 들어간다.** 한 줄 요약만 페이지 헤더 바로 아래로 올라간다. 신청 묶음은 하단 신청 바가 맡는다.
...
**한 줄 요약은 모바일에서 지도 띠 바로 아래로 올린다**(디자이너 판단 2026-09-24). 프로필 탭 안에 두면 소개글 전문 뒤에 요약이 한 번 더 나온다. 띠 아래에 두면 **요약이 전문보다 먼저** 온다.

**지도 띠는 신원 블록(아바타 · 이름 · 직무 · 도시)까지만 품는다**(2차 export 검수 뒤 디자이너 판단 2026-09-24). 2차 export 는 띠가 요약까지 품어 너무 높았다. 띠가 높으면 지도가 세로로 넓게 보여 멘토의 도시 주변 대신 먼 바다와 대륙이 크게 잡힌다. **핀은 띠의 오른쪽 · 세로 가운데**에 둔다.

**모바일 멘토 바는 탭 줄과, 그 위에 떠 있는 이름 줄이다.** 이름 줄은 흐름 안에 자리를 차지하지 않는다. 페이지 헤더가 화면에서 사라지면 위에서 내려와 탭 줄 위에 선다. 2차 export 는 이름 줄을 페이지 헤더 아래에 겹쳐 두는 방식이라 그 자리가 빈칸으로 남았다.
...
| 무엇 | 방문자 | 본인 |
|---|---|---|
| 신청 묶음(왼쪽 열 머리) · 신청 바 | 신청 ＋ 관심있는 멘토 ＋ 공유 | **프로필 수정하기 ＋ 공유.** 관심있는 멘토 등록은 뺀다 — 자기를 등록할 일이 없다 |
...
| **`P-03`** 공유는 단말에 따라 수단이 갈린다 | 신청 묶음과 신청 바의 공유. 와이어 결정사항의 「모바일은 OS 공유 모달, 웹은 popover → 링크 복사 / 공유하기」와 같다 |
...
| **미승격** 모바일에서 아래로 읽는 동안 전역 바(`Header` · `BottomTabBar`)가 비켜 준다 | 1화면째다. 이 화면만의 바(모바일 멘토 바 · 신청 바)가 남는다. `mentoring-apply` · `qna-compose` 에서 같은 판단이 나오면 올린다 |
...
| — **`DS-54`** | 페이지 헤더 | `Avatar` · 이름 · 직무 @회사 · 연차 · 국기 ＋ 「<도시>에서 활동」 · 세계지도 ＋ 핀 | 풀폭. 핀 고정 · 지도 이동(§4). **이름과 직무를 말줄임하지 않는다** — 줄을 늘린다. 모바일은 띠를 신원 블록까지만 두고 핀을 오른쪽 세로 가운데에 둔다 |
| 조립 | **왼쪽 열 머리** | 멘토 정보(`Avatar` · 이름 · 직무) ＋ 신청 묶음 | 처음에는 신청 묶음만 선다. **페이지 헤더가 사라지면 멘토 정보가 위에서 내려와 신청 묶음 위에 자리를 잡는다.** 신청 묶음은 계속 보인다 |
| `Button` ＋ `IconButton`(`pressed`) ＋ `IconButton` | 왼쪽 열 머리 · 신청 묶음 | 1:1 멘토링 신청하기(primary) · 관심있는 멘토 · 공유 | 관심있는 멘토는 **숫자 없는 켜짐·꺼짐 토글이고, 켜지면 반전한다** — 멘토카드의 `BookmarkToggle` 과 같은 모양이다(`DS-35`) |
| `Popover` | 공유 | 링크 복사 · 공유하기 | 데스크톱. 모바일은 OS 공유 시트(`P-03`) |
| 조립 | 한 줄 요약 | 멘토카드의 소개(`intro`)와 같은 값 | green-50 상자 — 멘토카드와 같다. 데스크톱은 왼쪽 열 머리 바로 아래, 모바일은 지도 띠 바로 아래 흰 바탕 |
...
| 요소명(English) | 종류 | 내용 | 이 화면에서의 규칙 |
|---|---|---|---|
| `Tabs` | 탭 바 | 프로필 · 인터뷰 · Q&A · 리뷰 | **데스크톱은 오른쪽 열 맨 위에서 따라온다. 모바일은 멘토 바의 탭 줄이다.** **Q&A 와 리뷰에 건수를 붙인다** — 답변과 리뷰의 양이 신청을 정하는 신뢰 신호다. 탭을 열지 않아도 보인다 |
...
| 요소명(English) | 종류 | 내용 | 이 화면에서의 규칙 |
|---|---|---|---|
| 조립 | 멘토 바 | 탭 줄 44 ＋ 그 위에 떠 있는 이름 줄 40 | 따라온다. **이름 줄은 흐름 안에 자리를 차지하지 않는다.** 페이지 헤더가 사라지면 위에서 내려온다 |
...
| 탭 에러 | 탭 본문 자리만 **「불러오지 못했어요」** ＋ **「다시 시도」** | 페이지 헤더 · 탭 바 · 왼쪽 열은 남긴다 |
...
| 탭 바가 화면 위에 붙은 채로 탭을 바꿨다 | **탭 본문의 머리로 스크롤한다.** 긴 인터뷰를 읽다가 리뷰로 바꾸면 리뷰 첫 줄이 보인다 |
...
| 「이어서 보기」를 눌렀다 | **인터뷰 탭으로 바꾸고, 탭 바가 화면 위에 붙는 자리까지 부드럽게 스크롤한다.** `prefers-reduced-motion` 이면 바로 옮긴다 |
...
| 판정 조건 | 아크션 |
|---|---|
| 본인 프로필이다 | 신청 묶음과 신청 바가 **「프로필 수정하기 ＋ 공유」**가 된다. 섹션 머리에 연필 · ＋, 아바타에 연필이 선다. **탭 끝 CTA 박스를 숨긴다** — 자기에게 신청할 일이 없다 |
...
| 판정 조건 | 아크션 |
|---|---|
| 데스크톱이다 | **왼쪽 열과 탭 바가 `Header` 아래에 붙는다. 둘의 윗선이 같다** |
| 페이지 헤더가 화면에서 사라졌다 · 데스크톱 | 왼쪽 열 머리에 **멘토 정보가 위에서 내려와 신청 묶음 위에 선다.** 신청 묶음 이하는 그만큼 내려간다 — 의도한 움직임이다. 탭 본문은 움직이지 않는다. 페이지 헤더가 다시 보이면 거둔다 |
| 페이지 헤더가 화면에서 사라졌다 · 모바일 | 멘토 바의 **이름 줄이 위에서 내려와 탭 줄 위에 선다.** 이름 줄은 흐름 밖이라 아래 콘텐츠를 밀지 않는다 |
| **왼쪽 열이 화면보다 길다** | 끝까지 스크롤한 뒤 아래에 붙는다 |
| **769~1024px 이다** | 왼쪽 열 280. 탭 바는 오른쪽 열 안에 다 들어간다 |
...
| 모바일이다 | 지도 띠는 신원 블록까지만. **핀은 띠의 오른쪽 · 세로 가운데** |
...
- `mentor-detail` → `mentoring-apply`: 신청 묶음 · 신청 바 · 탭 끝 CTA 박스의 「1:1 멘토링 신청하기」
...
- [x] `DS-35` 의 켜짐을 반전으로 고치고 `DS-68` 을 디자인 시스템에서 만든다 (2026-09-24)
- [x] 2차 export 를 검수하고 구성을 고친다 — 신청 묶음을 왼쪽 열 머리로 · 탭 바를 오른쪽 열로 · 모바일 지도 띠와 이름 줄 (2026-09-24)
- [ ] 3차 export — `DS-54` · `DS-57` · `DS-67` 은 `data-temp` 임시 조립
- [ ] `template/` 을 취임한다
