...
- 상태: 확정 (§1~5 UX승인 2026-09-24 → §6~11 확정 2026-09-24)
- 화면 구분: 열람
- 템플릿: 아직 없다. 아트디렉션을 TOP 에 시범 적용한 뒤 Claude Design 에서 만든다
...
**위에서 아래로 「누구인가 → 어떤 사람인가(인터뷰) → 믿을 만한가(답변·리뷰) → 신청」이다.** 신청은 이 흐름의 어디에서든 손이 닿게 따라온다.
...
| 순 | 블록 | 무엇을 하나 |
|---|---|---|
| 1 | `Header` | 전역 헤더다. 이 화면 고유의 구성은 없다 |
| 2 | **페이지 헤더** | **누구인가.** 아바타 · 이름 · 직무 @회사 · 연차 · 국가와 도시. 배경에 세계지도를 깔고 멘토의 위치에 핀을 찍는다 |
| 3 | **신청 묶음** | 1:1 멘토링 신청하기 ＋ 관심있는 멘토 ＋ 공유. **스크롤을 따라온다** |
| 4 | 왼쪽 열 | 인사이트 키워드 · 커리어 히스토리 · 학력 히스토리. **어느 탭에서든 보인다** |
| 5 | 탭 바 | 프로필 · 인터뷰 · Q&A · 리뷰. **스크롤을 따라온다** |
| 6 | 탭 본문 | 아래 「탭」 |
...
**신청 묶음은 모바일 하단 신청 바와 같은 셋이다.** 두 폭에서 같은 묶음이 따라온다. 탭 본문(인터뷰 전문·답변 목록)이 길어서, 묶음이 제자리에 머물면 읽는 동안 신청이 화면에서 사라진다.

### 탭

| 탭 | 무엇 | 0건일 때 |
|---|---|---|
| **프로필** | 인터뷰 표지(최신 1편) → 소개글(인용 ＋ 본문) → 멘토 N문N답 → 커리어 관련 활동 | N문N답 · 커리어 관련 활동은 **섹션을 숨긴다** |
...
**프로필 탭의 인터뷰 표지에서 「이어서 보기」를 누르면 인터뷰 탭으로 바꾼다.** 페이지를 떠나지 않는다.
...
탭 바 (따라옴)
탭 본문
─ 하단 고정: 신청 바 ／ BottomTabBar ─
```

**왼쪽 열이 없으므로 그 내용이 프로필 탭 안으로 들어간다.**

| 순 | 프로필 탭의 블록 | 왜 이 자리인가 |
|---|---|---|
| 1 | 인터뷰 표지(최신 1편) | 서비스의 특징이라 맨 앞이다 |
| 2 | 소개글 | |
| 3 | 인사이트 키워드 | **데스크톱의 왼쪽 열(3~5).** 데스크톱에서는 인터뷰·소개글과 같은 높이에서 함께 보인다. 한 줄로 펴면 그 바로 뒤다 |
| 4 | 커리어 히스토리 | |
| 5 | 학력 히스토리 | |
| 6 | 멘토 N문N답 | |
| 7 | 커리어 관련 활동 | |
...
**왼쪽 열의 내용은 한 화면에 한 번만 선다.** 데스크톱에서는 왼쪽 열에만 있고 프로필 탭에 다시 넣지 않는다. 모바일에서는 프로필 탭에만 있고 다른 탭(인터뷰·Q&A·리뷰)에는 나오지 않는다.

**하단은 두 줄로 고정한다.** 위가 신청 바(1:1 멘토링 신청하기 ＋ 관심있는 멘토 ＋ 공유), 아래가 `BottomTabBar` 다.

- **아래로 스크롤하면 `BottomTabBar` 가 숨고 신청 바가 바닥으로 내려온다.** 위로 스크롤하면 돌아온다
- **신청 바는 숨지 않는다**
...
| 무엇 | 방문자 | 본인 |
|---|---|---|
| 신청 묶음 · 신청 바 | 신청 ＋ 관심있는 멘토 ＋ 공유 | **프로필 수정하기 ＋ 공유.** 관심있는 멘토 등록은 뺀다 — 자기를 등록할 일이 없다 |
...
| **미승격** 고정 신청 바가 있는 모바일 화면은 아래로 스크롤할 때 `BottomTabBar` 를 숨긴다 | 1화면째다. `mentoring-apply` · `qna-compose` 에서 같은 판단이 나오면 올린다 |
...
**화면 위에서부터의 순서다.** 요소명은 [용어집](../../../docs/glossary/ubiquitous-language.md) English 열이다. 부품이 없는 자리는 「조립」 또는 「DS 후보」로 적었다 — 아래 「디자인 시스템에 없는 것」.
...
| — **DS 후보 ①** | 페이지 헤더 | `Avatar` · 이름 · 직무 @회사 · 연차 · 국기 ＋ 「<도시>에서 활동」 · 세계지도 ＋ 핀 | 풀폭. 핀 고정 · 지도 이동(§4). **이름과 직무를 말줄임하지 않는다** — 줄을 늘린다 |
| `Button` ＋ 토글 ＋ `IconButton` | 신청 묶음 | 1:1 멘토링 신청하기(primary) · 관심있는 멘토 · 공유 | 따라온다. 관심있는 멘토는 **숫자 없는 켜짐·꺼짐 토글**이다 → `DS-35` |
...
| 요소명(English) | 종류 | 내용 | 이 화면에서의 규칙 |
|---|---|---|---|
| `Tabs` | 탭 바 | 프로필 · 인터뷰 · Q&A · 리뷰 | 따라온다. **Q&A 와 리뷰에 건수를 붙인다** — 답변과 리뷰의 양이 신청을 정하는 신뢰 신호다. 탭을 열지 않아도 보인다 |
| `InterviewCard` | 프로필 · 인터뷰 표지 | 최신 1편 · 사진 · 제목 · 발췌 · 「이어서 보기」 | 누르면 인터뷰 탭으로 바꾼다. **모양은 아트디렉션**(매거진 표지) |
| 조립 | 프로필 · 소개글 | 인용 한 줄 ＋ 본문 | 멘토가 쓴다. 반드시 있다. 인용을 강조한다 |
| 조립 | 프로필 · 멘토 N문N답 | 질문과 답의 쌍 | **접지 않고 전부 펼친다.** 0건이면 숨긴다 |
| 조립 | 프로필 · 커리어 관련 활동 | 썸네일 · 출처(YouTube · brunch) · 제목 | 누르면 **외부 링크를 새 창으로** 연다. 0건이면 숨긴다 |
| — **DS 후보 ③** | 인터뷰 · 전문 | 머리(`Badge` ×2 · 제목 · 날짜) ＋ 본문 | `article-detail` 과 **같은 글 · 같은 폭 720**(§4) |
| `InterviewCard` ×N | 인터뷰 · 지난 인터뷰 | 최신을 뺀 나머지 | **2편 이상일 때만 선다.** 누르면 같은 탭에서 그 편으로 바꾼다 |
| — **DS 후보 ④** | Q&A · 한 건 | 질문 한 줄 · 답변 발췌 · 반응 줄(도움돼요 · 조회 · 스크랩) | 카드 전체가 `qna-detail` 로 가는 링크다. **모양은 아트디렉션** |
| `Button` | Q&A · 리뷰 · 더보기 | — | 5건씩 그 자리에 붙인다 |
| 조립 | 리뷰 · 한 건 | `Avatar`(이니셜) · 가린 이름 · 상대 날짜 · 제목 · 본문 | 별점 없음(§3). 본문은 4줄을 넘으면 말줄임 ＋ 「더보기」 |
| 배너 | 탭 끝 CTA 박스 | 「이 멘토와 직접 이야기 나누고 싶나요?」 ＋ 1:1 멘토링 신청하기 | **모든 탭의 끝.** `DS-34` 의 배너다. 문구가 2줄이다 |
| `Footer` | 전역 셸 | — | |
...
| `BottomTabBar` — **DS 후보 ②** | 하단 탭바 | — | 아래로 스크롤하면 숨는다(§8) |

### 디자인 시스템에 없는 것

**번호는 기표할 때 받는다.** 원격 브랜치 전부의 `DS-update-list.md` 를 보고 매긴다.

| 후보 | 무엇이 없나 | 판정 |
|---|---|---|
| **①** 페이지 헤더(지도) | 풀폭 밴드 ＋ 세계지도 ＋ 고정 핀 ＋ 신원 블록. 도시에 맞춰 지도를 민다 | **기표한다.** 화풍은 아트디렉션에서 정한다 |
| **②** `BottomTabBar` 의 스크롤 숨김과 위에 얹는 칸 | 스크롤로 숨는 거동이 없다. 신청 바를 얹을 자리가 없다 | **기표한다.** 신청 바를 칸에 얹어 한 부품이 두 줄을 함께 움직인다. 숨김은 켜는 화면에서만 동작한다 — 다른 화면은 그대로다 |
| **③** 아티클 본문 | `article-detail` 은 본문을 화면에서 조립했다 | **기표한다.** 두 화면이 같은 글을 같게 그려야 한다. 2화면째다(원칙 7) |
| **④** Q&A 답변 강조형 | `QnaCard` 는 질문이 본체다. `AnswerCard` 는 링크가 없다 | **기표한다.** 아트디렉션의 Q&A 개편에 넣는다 |

**기존 항목에 덧붙이는 것**: `DS-34`(배너)에 **문구 2줄** · `DS-35`(숫자 없는 토글)에 **관심있는 멘토**. 둘 다 #32 의 번호다.
...
| 탭 에러 | 탭 본문 자리만 **「불러오지 못했어요」** ＋ **「다시 시도」** | 페이지 헤더 · 왼쪽 열 · 탭 바는 남긴다 |
...
| 로그인한 사람이 관심있는 멘토를 눌렀다 | 켜짐 · 꺼짐을 바꾸고 **`Toast` ＋ 실행취소**를 낸다. `mentor-search` 의 스크랩과 같다 |
| 공유를 눌렀다 | `P-03` |
...
| 판정 조건 | 아크션 |
|---|---|
| 본인 프로필이다 | 묶음과 신청 바가 **「프로필 수정하기 ＋ 공유」**가 된다. 섹션 머리에 연필 · ＋, 아바타에 연필이 선다. **탭 끝 CTA 박스를 숨긴다** — 자기에게 신청할 일이 없다 |
...
| 판정 조건 | 아크션 |
|---|---|
| 데스크톱이다 | 신청 묶음과 탭 바가 **`Header` 아래에 붙어 따라온다.** 둘의 윗선을 맞춘다 |
| 모바일에서 아래로 스크롤한다 | `BottomTabBar` 가 숨고 신청 바가 바닥으로 내려온다 |
| 모바일에서 위로 스크롤한다 · 페이지 맨 위다 | `BottomTabBar` 가 돌아온다 |

### 페이지 헤더

| 판정 조건 | 아크션 |
|---|---|
| 도시가 있다 | 그 도시를 핀 아래로 가져온다 |
...
| API (용도) | 계약 |
|---|---|
| 멘토 1건 — 기본정보 · 소개글 · 인사이트 키워드 · 커리어 · 학력 · N문N답 · 커리어 관련 활동 · 도시 좌표 · 본인 여부 | **계약 미정의.** 404 와 비공개를 가려 준다 |
...
- [ ] DS 후보 ①~④ 를 `DS-update-list.md` 에 기표하고 `DS-34` · `DS-35` 에 덧붙인다 — 번호를 정리한 뒤
- [ ] 아트디렉션을 TOP 에 시범 적용한 뒤 `template/` 을 Claude Design 에서 만들어 취임한다
