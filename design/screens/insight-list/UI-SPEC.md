...
| 5 | 아티클 목록 | 이 화면의 본체. 카드는 **가로형** — 왼쪽 글, 오른쪽 썸네일 |
| 6 | 페이지 이동 | |
...
**모바일(768 이하).** 좌측 단이 없어진다. 카테고리는 제목 아래 **가로 탭**으로 선다(§11 질문 4). 조건은 국가·키워드 한 줄 ＋ 검색창 한 줄이다. 카드는 **세로형 부품 그대로**다 — 가로형은 좁은 폭에서 성립하지 않는다.
...
| # | 무엇 | 추천 |
|---|---|---|
| 1 | **범위** — 목록만인가, 아티클 상세까지인가, 공지사항 2장도 넣는가 | 목록 ＋ 상세 |
| 3 | **카드의 태그 색** — Q&A 카드와 같게(국기 배지 ＋ 뉴트럴)인가, 부품대로 컬러인가 | Q&A 와 같게 |
| 4 | **모바일 카테고리** — 가로 탭인가, 드롭다운인가 | 가로 탭 |
