...
원본은 Claude Design 캔버스의 export 이고 프로젝트 루트를 기준으로 경로를 잡는다. 리포에 배치하면서 [`place-insight.sh`](../../../_import/place-insight.sh)(커밋 안 됨)가 **경로만** 돌렸다. 2판 때 댔던 카테고리 sticky 는 3판 원본에 들어와 지웠다. 화면의 내용은 고치지 않았다.
