...
- 상태: 확정 (§1~11 ＋ `template/` 취임 2026-09-14 · **2026-09-15 개정** — 검색 결과 상태 · 0건 화면의 위젯)
- 화면 구분: 열람
...
우측 위젯은 **질문하기 CTA → 나의 Q&A → 주목받는 태그로 찾기** 순이다. 상시 노출한다. **검색 결과 0건일 때만 우측을 통째로 감춘다**(2026-09-15 결정 — 와이어 `1059:37433`).
...
| 패턴 | 이 화면에서의 적용 |
|---|---|
| **`P-04`** 목록의 상태를 URL 에 담고 뒤로가기로 되돌린다 | 국가·키워드·전환·정렬·페이지를 URL 에 담는다. 필터를 바꾸면 1페이지로 되돌린다. 페이지를 옮기면 목록 머리로 스크롤한다. **2026-09-16 에 승격됐다**(2화면째 `insight-list`) |
...
| 2 | 나의 Q&A — **박스** ＋ `Tabs`(`DS-02`) ＋ 카드 미니 목록 ＋ 더보기 | 탭 구성이 **역할로 갈린다**(§8). 탭 문구는 「내가 질문한 멘토링 / 스크랩한 멘토링」(와이어). 더보기는 primary. **0건이면** 「아직 질문이 없으시네요!」 ＋ 「지금 바로 질문하기」(와이어 `1059:35645`) |
| 3 | 주목받는 태그로 찾기 — **박스** ＋ `Chip` ×5 | 누르면 **그 태그로 검색한 결과**가 된다. 단어 검색이 아니라 태그 검색이다. **제목과 칩을 가운데 정렬한다**(와이어 `1059:36073`). 개수는 5개다 |

**세 블록을 전부 테두리 박스에 담는다**(2026-09-15 결정). 싸개는 `Card` 를 그대로 쓴다 — 새 부품을 만들지 않는다(`DS-31` 취하). 「나의 Q&A멘토링」만 안에 카드가 들어가 **테두리가 두 겹**이 된다. 그 자리의 동심원 값은 어긋난 채로 둔다.
...
|---|---|---|
| 정상 | 위 §6 그대로 | |
| **검색 결과** | 「검색된 질문 리스트 N건」 ＋ 카드. **주목받는·전환·목록 정렬·쪽 이동을 감춘다.** 위젯은 그대로 | 와이어 `1059:36991`. 태그를 눌러도 이 상태다 |
...
| 어느 경우 | 무엇을 낸다 |
|---|---|
| **검색·필터의 결과가 0건** | 「찾으시는 질문이 없습니다.」 ＋ 「Q&A 멘토링 질문하기」 ＋ 작은 3줄 「구체적인 고민이 있나요? / 현재 가지고 있는 고민을 바로 물어보세요. / 멘토님들이 답변해드릴거에요」. **우측 위젯을 통째로 감춘다**(2026-09-15 결정) |
...
| **답변이 0건인 질문을 스크랩했다** | `Toast` 「답변이 달리면 알람을 받을까요?」 ＋ 액션 2개(와이어 `1059:35248`). **응답이 없으면 받지 않음으로 둔다** |
