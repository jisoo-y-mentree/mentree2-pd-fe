...
**이 화면이 1화면째다.** 세 건은 **`mentor-search` 가 2화면째로 실증해 승격됐다**(2026-09-12).

| 패턴 | 이 화면에서의 적용 |
|---|---|
| **[`P-01`](../../UX-PATTERNS.md)** 목록의 상태를 URL 에 담는다 | 국가·키워드·전환·정렬·페이지를 URL 에 담는다. 필터를 바꾸면 1페이지로 되돌린다. 페이지를 옮기면 목록 머리로 스크롤한다 |
| **[`P-02`](../../UX-PATTERNS.md)** 비로그인이 로그인이 필요한 조작을 눌렀다 | 도움돼요·스크랩에서 로그인으로 보낸다. 읽기와 공유는 막지 않는다 |
| **[`P-03`](../../UX-PATTERNS.md)** 0건의 면에서 다음 행동을 낸다 | 찾은 결과가 0건이면 질문 쓰기로 보낸다 |

**판단은 [UX-PATTERNS.md](../../UX-PATTERNS.md) 가 갖는다.** 여기에는 이 화면에서의 적용값만 남긴다.
...
| 5 | **에러와 빈 상태의 문구** | UX 라이팅이며 **한/일 2개국어**다. 문구를 직접 쓰지 않고 키로 짠다 | `DS-11` ／ UX 라이팅 착수 |
