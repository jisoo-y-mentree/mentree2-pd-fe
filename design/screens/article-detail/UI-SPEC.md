...
| 4′ | **「목록으로 돌아가기」** — 배너 아래 가운데 | 읽고 나서 다음 글을 고르러 가는 길. 상세 전부에 있다(2026-09-16 · 상혁) |
| 5 | **카테고리별 부속 블록** — A 참여 멘토 4장 ／ B 관련 아티클 4장 ／ C 멘토 상세로 | 더 읽을 것, 더 만날 사람. **4장짜리는 `Carousel` 부품에 담는다** — 모바일 거동은 부품이 정한다 |
...
**모바일(768 이하).** 3단이 1단이 된다. 좌 레일은 **제목 아래 한 줄**로 내려온다 — Q&A 상세의 모바일과 같은 자리다. 우 목차는 **숨긴다.** 본문 안의 H2 가 그 일을 한다. 배너는 문구 위 · 버튼 아래로 쌓이고 「목록으로 돌아가기」는 그 아래 가운데다. **관련 아티클 4장과 참여 멘토 4장은 `Carousel` 부품에 담는다**(2026-09-16 상혁). 모바일에서 옆으로 넘기는 것은 부품의 지침(`03-responsive.md` 캐러셀)이 정한다 — 화면이 따로 정하지 않는다. 구독 플로팅은 하단 탭바 위에 좌우 꽉 차게 뜬다.
...
| **`P-05`** 없는 글의 URL 은 404 화면으로 받는다 | 존재하지 않거나 비공개인 아티클은 「글을 찾을 수 없어요」 ＋ 「멘트리 인사이트로」. **2026-09-17 승격**(2화면째 `notice-detail`) |
...
| 「목록으로 돌아가기」 | 부품 | `Button outline md` ＋ `arrow-left-01` | 배너 아래 가운데. 목록에서 왔으면 **떠난 목록 상태로**(`history.back` · `P-04`), 아니면 목록 첫 화면으로. 모바일에서도 폭 100% 가 아니다 — 배너 버튼과 구분 |
| 부속 블록 — A | 부품 | `SectionHeader`(화살표) 「참여 멘토」 ＋ **`Carousel`**(mode peek) 에 `MentorCard` ×4 | 에디터가 지정. **공란이면 블록이 없다.** `MentorCard` 는 데스크톱 296 고정이라 4장이 컨테이너 1232 에 들지 않는다 — DS 캐러셀 규칙(「멘토는 peek」)대로 `Carousel` 에 담는다(2026-09-16 감사 · 2판 template 반영) |
| 부속 블록 — B | 부품 | `SectionHeader`(화살표) 「관련 아티클 추천」 ＋ **`Carousel`**(mode snap-fit) 에 `ArticlePreview` ×4 | 세로형. **발췌 · 태그를 넘기지 않는다** — 썸네일 ＋ 제목만. 2026-09-16 상혁 · 2판 template 반영 |
...
| **스크롤이 70% 를 지났다** — 배너가 보이기 시작하는 자리 | 구독 카드를 낸다. 한 번만. **어느 상자가 스크롤되든** 잡아야 한다(2026-09-17 · `IntersectionObserver` 표지) |
| 구독 카드를 **닫았다** | **이 글에서는** 이 세션 동안 다시 내지 않는다. 다른 글에서는 낸다(2026-09-17 정정 — 탭 단위로 기억하면 데모에서 두 번째 글부터 영영 안 뜬다) |
...
| **768 이하다** | 레일은 제목 아래 가운데 한 줄. 목차 숨김. 배너는 세로 쌓임. 관련 아티클 · 참여 멘토는 `Carousel` 부품 그대로. 구독 카드는 하단 탭바 위 좌우 꽉 |
| 「목록으로 돌아가기」를 눌렀다 | 목록에서 들어왔으면 `history.back()` — 조건 · 페이지가 살아 있는 목록으로(`P-04`). 공유 링크 · 검색으로 바로 들어왔으면 목록 첫 화면으로 |
...
- `article-detail` → `insight-list`: ← · 브레드크럼 · 배너 아래 「목록으로 돌아가기」. 목록에서 왔으면 떠난 상태로 되돌아간다
...
| 7 | ~~참여 멘토 · 관련 아티클을 `Carousel` 로, 「목록으로 돌아가기」 추가~~ | **풀렸다** — 2판 template(2026-09-17)에 들어왔다 | — |
| 8 | ~~구독 카드가 안쪽 스크롤 상자에서 안 뜬다~~ | **풀렸다** — 3판 template(2026-09-17 14:09)이 `IntersectionObserver` 표지 ＋ 글 단위 기억으로 바꿨다 | — |
