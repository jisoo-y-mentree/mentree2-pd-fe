...
| 2 | **머리** — 태그(국가 · 키워드) · 제목 · 발행일. 가운데 정렬 | 무슨 글인지 한 눈에 |
...
**사람이 답해야 하는 것 — 2026-09-16 에 물었다.**

| # | 무엇 | 추천 |
|---|---|---|
| 2 | **이벤트 상세** — 이 화면의 A형이 이벤트 상세인가, 따로 만드는가(와이어 없음) | A형이 맡는다. 주석이 A형에 「행사 참여신청 ＋ 참여 멘토」를 이미 그렸다 |

**아직 사실이 없는 것.**

- 「관련 멘토 보기」 모달의 화면 — 추천 규칙은 주석에 있고 화면은 없다
- 「구독」이 무엇인가 — 이메일인가, 사이트 알림인가. 사실이 없다. 문구는 와이어대로 둔다
- 「참여 신청하기」가 어디로 가는가 — 이벤트 신청의 화면이 없다
