...
| 5 | **카테고리별 부속 블록** — A 참여 멘토 4장 ／ B 관련 아티클 4장 ／ C 멘토 상세로 | 더 읽을 것, 더 만날 사람. **4장짜리는 `Carousel`** — 모바일에서 옆으로 넘긴다(`P-05`) |
...
**모바일(768 이하).** 3단이 1단이 된다. 좌 레일은 **제목 아래 한 줄**로 내려온다 — Q&A 상세의 모바일과 같은 자리다. 우 목차는 **숨긴다.** 본문 안의 H2 가 그 일을 한다. 배너는 문구 위 · 버튼 아래로 쌓이고 「목록으로 돌아가기」는 그 아래 가운데다. **관련 아티클 4장과 참여 멘토 4장은 세로로 쌓지 않고 옆으로 넘긴다**(`Carousel` 스와이프 · 2026-09-16 상혁 · `P-05`). 세로로 쌓으면 4장이 화면 두 장을 먹는다. 구독 플로팅은 하단 탭바 위에 좌우 꽉 차게 뜬다.
...
| 부속 블록 — A | 부품 | `SectionHeader`(화살표) 「참여 멘토」 ＋ **`Carousel`**(mode peek) 에 `MentorCard` ×4 | 에디터가 지정. **공란이면 블록이 없다.** `MentorCard` 는 데스크톱 296 고정이라 4장이 컨테이너 1232 에 들지 않는다 — DS 캐러셀 규칙(「멘토는 peek」)대로 `Carousel` 에 담는다(2026-09-16 감사). 모바일은 스와이프(`P-05`). 1판 template 은 가로 스크롤 줄이다 |
| 부속 블록 — B | 부품 | `SectionHeader`(화살표) 「관련 아티클 추천」 ＋ **`Carousel`**(mode snap-fit) 에 `ArticlePreview` ×4 | 세로형. **발췌 · 태그를 넘기지 않는다** — 썸네일 ＋ 제목만. 데스크톱은 4장이 다 들어 화살표가 둘 다 disabled. **모바일은 스와이프**(`P-05` · 2026-09-16 상혁). 1판 template 은 세로 1열이다 |
...
| **768 이하다** | 레일은 제목 아래 가운데 한 줄. 목차 숨김. 배너는 세로 쌓임. **관련 아티클 · 참여 멘토는 옆으로 넘긴다**(`Carousel` 스와이프 · `P-05`). 구독 카드는 하단 탭바 위 좌우 꽉 |
...
| 7 | **참여 멘토 · 관련 아티클을 `Carousel` 로, 「목록으로 돌아가기」 추가** | 1판 export 에 없다. 배치 판은 참여 멘토를 가로 스크롤로 막았고 관련 아티클은 세로 1열이다. `insight-round2.md` 1 · 2 · 3 | 다음 export |
